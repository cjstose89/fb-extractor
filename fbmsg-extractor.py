...
#
# Modified by sunavetsennoloc - 27 Agustus 2018
...
if (options.contactstsv != None):
    try:
        contactsof = open(options.contactstsv, "w", encoding="utf-8")
    except:
...
threadsquery = "select messages.msg_id, messages.thread_key, messages.text, messages.sender, threads.thread_key, messages.timestamp_ms, messages.source, messages.coordinates from messages, threads where messages.thread_key=threads.thread_key order by messages.thread_key, messages.timestamp_ms;"
...
if (options.messagestsv != None):
    try:
        messagesof = open(options.messagestsv, "wb")
    except:
...
# write header for contacts output file
if (options.messagestsv != None):
    messagesof.write(str.encode("msg_id\tthread_id\ttext\tsender\tparticipants\ttimestamp_ms\tsource\tlatitude\tlongitude\taccuracy\theading\tspeed\taltitude\tgooglemaps\n"))
...
for key in sorted_messages_keys:
    ...
    if (options.messagestsv != None):
        messagesof.write(str.encode(key + "\t" + messagesdict[key][MESGS_THR_ID_IDX] + "\t" + \
        messagesdict[key][MESGS_TEXT_IDX] + "\t" + messagesdict[key][MESGS_SENDER_IDX] + \
        "\t" + partieslist + "\t" + datetimestr + "\t" + messagesdict[key][MESGS_SOURCE_IDX] + \
        "\t" + str(messagesdict[key][MESGS_LAT_IDX]) + "\t" + str(messagesdict[key][MESGS_LONG_IDX]) + \
        "\t" + str(messagesdict[key][MESGS_ACCURACY_IDX]) + "\t" + str(messagesdict[key][MESGS_HEADING_IDX]) + \
